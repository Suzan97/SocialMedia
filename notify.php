        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <ul class="list-group">
                      <?php
                      if (DB::query('SELECT * FROM notifications WHERE receiver=:user_id', array(':user_id'=>$user_id))) {
                              $notifications = DB::query('SELECT * FROM notifications WHERE receiver=:user_id', array(':user_id'=>$user_id));
                              foreach($notifications as $n) {

                                if($n['type'] == 1){
                                 $senderName = DB::query('SELECT username FROM users WHERE user_id=:senderid', array(':senderid'=>$n['sender']))[0]['username'];
                                   if($n['extra'] == ""){
