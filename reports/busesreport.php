...
<div class="form-style-2-heading">Buses</div>
<?php

$sql ="SELECT * FROM buses ".$filterquery1." ";
...
if ($result ->num_rows>0){
	...
	while($row =$result->fetch_assoc()){
		...
		$sql2 ="SELECT COUNT(bus_id) AS countof FROM trips WHERE bus_id=".$row["bus_id"]." ";
...
array_push($arrayof,$row2["countof"]);		
		...
		array_push($arrayof2,$row["bus_name"]);
	}
	$listofbusesbookingsarray =  implode(', ',$arrayof);
	$listofbusesarray =  '"'.implode('", "',$arrayof2).'"';
	echo"</tbody>
	</table>
	</div>
	</div>
...
var myChart = new Chart(ctx, {
    type: 'bar',
    data: {
        labels: [<?php echo $listofbusesarray; ?>],
        datasets: [{
            label: '# of Bookings',
            data: [<?php echo $listofbusesbookingsarray; ?>],
            backgroundColor: [
                'rgba(255, 99, 132, 0.2)',
                'rgba(54, 162, 235, 0.2)',
                'rgba(255, 206, 86, 0.2)',
