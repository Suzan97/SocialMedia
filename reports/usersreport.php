...
$sql ="SELECT * FROM users ".$filterquery1." ";
...
if ($result ->num_rows>0){
	...
	while($row =$result->fetch_assoc()){
		...
		$sql2 ="SELECT COUNT(user_id) AS countof FROM bookings WHERE user_id=".$row["user_id"]." ";
...
array_push($arrayof,$row2["countof"]);		
		...
		array_push($arrayof2,$row["user_name"]);
	}
	$listofusersbookingsarray =  implode(', ',$arrayof);
	$listofusersarray =  '"'.implode('", "',$arrayof2).'"';
	echo"</tbody>
	</table>
	</div>
	</div>
...
var myChart = new Chart(ctx, {
    type: 'bar',
    data: {
        labels: [<?php echo $listofusersarray; ?>],
        datasets: [{
            label: '# of Bookings',
            data: [<?php echo $listofusersbookingsarray; ?>],
            backgroundColor: [
                'rgba(255, 99, 132, 0.2)',
                'rgba(54, 162, 235, 0.2)',
                'rgba(255, 206, 86, 0.2)',
